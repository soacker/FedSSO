...
def read_data(dataset, idx, is_train=True):
    if is_train:
        ...
        # train_file = train_data_dir + 'train' + str(idx) + '_.npy'
        ...
        # test_file = test_data_dir + 'test' + str(idx) + '_.npy'
        # test_data = np.load(test_file, allow_pickle=True).tolist()

        return test_data


def __read_client_data(dataset, idx, is_train=True):
    ...
    if is_train:
        ...
        # train_data = [(one_hot_encode(x), y) for x, y in zip(X_train, y_train)]
        return train_data
    else:
        ...
        # test_data = [(one_hot_encode(x), y) for x, y in zip(X_test, y_test)]
        return test_data


def read_client_data(dataset, idx, is_train=True):
    return __read_client_data(dataset, idx, is_train=True), __read_client_data(dataset, idx, is_train=False) 
...
def read_client_data_text(dataset, idx, is_train=True):
    if is_train:
        ...
        train_data = [((one_hot_encode(x), lens), y) for x, lens, y in zip(X_train, X_train_lens, y_train)]
        return train_data
    else:
        test_data = read_data(dataset, idx, is_train)
        ...
        test_data = [((one_hot_encode(x), lens), y) for x, lens, y in zip(X_test, X_test_lens, y_test)]
        return test_data

def one_hot_encode(arr, n_labels=65):
