...
class MemReporter():
    ...
    def print_stats(self, verbose: bool = False, target_device: Optional[torch.device] = None) -> dict:
        results = {}
        # header
        # show_reuse = verbose
        ...
        for device, tensor_stats in self.device_tensor_stat.items():
            ...
            if device == torch.device('cpu'):
                results['cpu_tensors'] = total_numel
                results['cpu_memory'] = total_mem
            elif device == torch.device('cuda'):
                results['gpu_tensors'] = total_numel
                results['gpu_memory'] = total_mem
                results['gpu_alloc'] = torch.cuda.memory_allocated()
            ...
            if device != torch.device('cpu'):
                ...
                # added by liub
                results['allocated_cuda_memory'] = memory_allocated
    ...
    def report(self, verbose: bool = False, device: Optional[torch.device] = None) -> dict:
