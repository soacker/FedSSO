...
class server_adagrad_paper_2023(Server):
    def __init__(self, device, dataset, algorithm, model, batch_size, learning_rate, global_rounds, local_steps,
        ...
        self.opimzer_list.append(
            Optimizer_adagrad(self.global_model, 0, len(select_layers), model_length,
...
class Optimizer_adagrad(Optimizer_SSO):
    def __init__(self, model, from_layer, to_layer, model_length, select_layers, device, learning_rate, config=None):
        super(Optimizer_adagrad, self).__init__(model, from_layer, to_layer, model_length, select_layers, device,
                                                learning_rate)

        # self.grad = copy.deepcopy(model)
        assert config != None
        self.adagrad = torch.optim.Adagrad(self.model.parameters(), lr=config.server_learning_rate, weight_decay=0,
                                           eps=0, initial_accumulator_value=0)
