...
class server_yogi_paper_2023(Server):
...
class Optimizer_yogi(Optimizer_SSO):
    def __init__(self, model, from_layer, to_layer, model_length, select_layers, device, learning_rate, config=None):
        super(Optimizer_yogi, self).__init__(model, from_layer, to_layer, model_length, select_layers, device,
                                            learning_rate)

        # self.grad = copy.deepcopy(model)
        # self.adagrad = torch.optim.Adagrad(self.model.parameters(), lr=0.01)
        from ..optimizers.torch_optimizer import Yogi
        assert config != None
        self.yogi = Yogi(self.model.parameters(), lr=config.server_learning_rate, weight_decay=0, eps=0, initial_accumulator=0)
